...
const logoutUser = asyncHandler(async (req, res) => {
    ...
    await User.findByIdAndUpdate(
        req.user._id,
        {
            $unset: { refreshToken: 1 } /* This is the operator of mongodb to update and set something so we have pass in form of object that is refreshToken and set it to 1 which is true. We can write undefined or null in set but this unset approach is better. */
        },
        {
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    /* If your Cloudinary URL looks like:
        https://res.cloudinary.com/<cloud_name>/image/upload/v1234567890/folder/image_name.jpg
    The public ID is everything after /upload/ and before the file extension:
        For the above example: folder/image_name */
    try {
        
        function getPublicIdFromUrl(url) {
            // 1. Removing query params from url 
            url = url.split('?')[0];
            
            // 2. Extracting relevant part of url after upload and getting it
            const publicIdWithVersion = url.split('/upload/')[1];

            // 3. Getting id without version
            const publicIdwithoutVersion = publicIdWithVersion.replace(/v\d+\//,'');

            // 4. Removing file extension and getting actual pubilic id
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    // ASSIGNMENT = Create a fuction for deleting old cover image
    // Getting old avatar url form db
    const currentUser = await User.findById(req.user?._id);
    const coverImageUrl = currentUser?.coverImage;
    ...
    // Deleting old cove image from cloudinary
    try {
